...
#[wasm_bindgen]
pub fn toprf_server_eval_wasm(blinded: String, share: String) -> Result<String, JsValue> {
    let blinded = b64_decode_array::<32>(&blinded).map_err(map_err)?;
    let share = b64_decode_array::<32>(&share).map_err(map_err)?;
    let y = upspa_core::toprf::toprf_server_eval(&blinded, &share).map_err(map_err)?;
    Ok(b64_encode(&y))
}
#[derive(Deserialize)]
