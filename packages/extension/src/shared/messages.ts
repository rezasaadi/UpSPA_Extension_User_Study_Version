...
export type BgRequest =
  ...
  | { type: 'UPSRA_SECRET_UPDATE_PREP'; lsj: string; password: string; website_password: string }
...
export type BgResponse =
  ...
  | {
      ok: true;
      credential:
        | { credential_kind: 'derived'; vinfo_prime_b64: string; best_ctr: number }
        | { credential_kind: 'embedded_password'; website_password: string; best_ctr: number };
    }
  | {
      ok: true;
      secret_update: {
        credential_kind: 'embedded_password';
        previous_credential_kind: 'derived' | 'embedded_password';
        cj_new: CtBlobB64;
