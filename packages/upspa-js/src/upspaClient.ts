...
export class UpspaClient {
  ...
  async setupAndProvision(password: string, tsp = this.threshold): Promise<SetupResult> {
    ...
    const results = await Promise.allSettled(
      out.sp_payloads.map((p) => this.spById(p.sp_id).setup(p)),
    );
    ...
    if (ok < this.threshold) {
      const errs = results
        .map((r, i) =>
          r.status === 'rejected'
            ? `sp_id=${out.sp_payloads[i].sp_id}: ${String(r.reason)}`
            : null,
        )
        .filter(Boolean)
        .join('\n');
      ...
      throw new Error(
        `Setup provisioning succeeded on ${ok}/${nsp} SPs (< threshold ${this.threshold}).\n${errs}`,
      );
    }
  ...
  async deriveStateKey(
    password: string,
  ): Promise<{ state_key_b64: string; begin: ToprfBegin; partials: ToprfPartial[] }> {
    await this.init();

    const begin = this.w().toprf_begin(password) as ToprfBegin;

    const evals = await Promise.allSettled(
      this.sps.map((sp) => sp.toprfEval(this.uid, begin.blinded)),
    );
    ...
    if (partials.length < this.threshold) {
      const errors = evals
        .map((r, i) =>
          r.status === 'rejected'
            ? `sp_id=${this.sps[i].id}: ${String(r.reason)}`
            : null,
        )
        .filter(Boolean)
        .join('\n');
      ...
      throw new Error(
        `TOPRF: only ${partials.length}/${this.sps.length} partials succeeded (< threshold ${this.threshold}).\n${errors}`,
      );
    }
  ...
  async prepareRegistration(lsj: string, password: string): Promise<RegistrationOut> {
    await this.init();

    const { state_key_b64 } = await this.deriveStateKey(password);
    const cid = await this.fetchCid();

    return this.w().protocol_register(
      this.uid,
      lsj,
      state_key_b64,
      cid,
  ...
  async prepareMigration(
    ...
    const { state_key_b64 } = await this.deriveStateKey(masterPassword);
    const cid = await this.fetchCid();

    return this.w().protocol_migrate_existing(
      this.uid,
      lsj,
      state_key_b64,
      cid,
  ...
  async applyRegistrationToSPs(records: RegistrationSpOut[]): Promise<void> {
    const writes = await Promise.allSettled(
      records.map((m) => this.spById(m.sp_id).createRecord(m.suid, m.cj)),
    );
    ...
    if (ok < this.threshold) {
      throw new Error(
        `Registration SP writes: only ${ok}/${records.length} succeeded (< threshold ${this.threshold}).`,
      );
    }
  }
  ...
  async applyMigrationToSPs(records: RegistrationSpOut[]): Promise<void> {
    await this.applyRegistrationToSPs(records);
  }
  ...
  async migrateExisting(
    ...
    const out = await this.prepareMigration(lsj, masterPassword, websitePassword);
    await this.applyMigrationToSPs(out.per_sp);
    return out;
  }
  ...
  async authenticate(lsj: string, password: string): Promise<AuthFinishOut> {
    ...
    const prep = this.w().protocol_auth_prepare(
      this.uid,
      lsj,
      state_key_b64,
      cid,
    ...
    if (cjs.length < this.threshold) {
      throw new Error(
        `Authentication: only ${cjs.length}/${prep.per_sp.length} cj reads succeeded (< threshold ${this.threshold}).`,
      );
    }
  ...
  async secretUpdate(
    ...
    masterPassword: string,
    websitePassword: string,
  ): Promise<SecretUpdateFinishOut & { suids: Array<{ sp_id: number; suid: string }> }> {
    await this.init();

    const { state_key_b64 } = await this.deriveStateKey(masterPassword);
    const cid = await this.fetchCid();

    const prep = this.w().protocol_secret_update_prepare(
      this.uid,
      lsj,
      state_key_b64,
      cid,
      this.sps.length,
    ) as SecretUpdatePrepareOut;
    ...
    if (cjs.length < this.threshold) {
      throw new Error(
        `Secret update: only ${cjs.length}/${prep.per_sp.length} cj reads succeeded (< threshold ${this.threshold}).`,
      );
    }

    const out = this.w().protocol_secret_update_finish(
      this.uid,
      lsj,
      prep.k0,
      cjs,
      websitePassword,
    ) as SecretUpdateFinishOut;

    return { ...out, suids: prep.per_sp };
  }

  async applySecretUpdateToSPs(
    suids: Array<{ sp_id: number; suid: string }>,
    cj_new: CtBlobB64,
  ): Promise<void> {
    const writes = await Promise.allSettled(
      suids.map((m) => this.spById(m.sp_id).updateRecord(m.suid, cj_new)),
    );
    ...
    if (ok < this.threshold) {
      throw new Error(
        `Secret-update SP writes: only ${ok}/${suids.length} succeeded (< threshold ${this.threshold}).`,
      );
    }
  }
  ...
  async passwordUpdate(
    ...
    if (ok < this.threshold) {
      throw new Error(
        `Password update: only ${ok}/${out.per_sp.length} SP updates succeeded (< threshold ${this.threshold}).`,
      );
    }
