...
const users = new Map();
const registrationConfirmations = new Map();
...
async function handleRegister(req, res) {
  ...
  const flowId = String(form.get("upspa_flow_id") || "").trim();
  const origin = String(form.get("upspa_origin") || "").trim();
  const nonce = String(form.get("upspa_confirmation_nonce") || "").trim();
  ...
  if (flowId && origin && nonce) {
    registrationConfirmations.set(flowId, {
      accountId,
      origin,
      nonce,
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/upspa/registration-status") {
      ...
      const flowId = String(url.searchParams.get("flowId") || url.searchParams.get("flow_id") || "").trim();
      const origin = String(url.searchParams.get("origin") || "").trim();
      const nonce = String(url.searchParams.get("nonce") || "").trim();
      const confirmation = flowId ? registrationConfirmations.get(flowId) : undefined;
      const bound =
        Boolean(confirmation) &&
        confirmation.origin === origin &&
        confirmation.nonce === nonce &&
        (!accountId || confirmation.accountId === accountId);
      ...
      return sendJson(res, 200, {
        ...
        origin,
        nonce,
        registered: bound && users.has(confirmation.accountId),
        confirmed: bound && users.has(confirmation.accountId),
      });
    }
